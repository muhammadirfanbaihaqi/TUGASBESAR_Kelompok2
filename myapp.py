...
def checkinOwner():
    ...
    if request.method == 'POST':
        ...
        Owner.checkin(nik, lantai_ke, kategori, nama_pelanggan, durasi, id_petugas, waktu_checkin, id_booking)
...
def booking_listOwner():
    ...
    booking_list_items = Owner.lihatBooking(cursor)
    ...
    if request.method == 'POST':
        Owner.checkout(conn, cursor)

        # Mengambil data booking setelah checkout
        booking_list_items = Owner.lihatBooking(cursor)
...
@app.route('/cetak_struk/<int:booking_id>')
def cetak_struk(booking_id):
    buffer = Struk.cetak(booking_id)
...
def room_listOwner():
    ...
    return Owner.melihat_daftar_kamar()
...
def booking_list():
    ...
    booking_list_items = []
    booking_list_items = Petugas.lihatBooking(cursor)
    ...
    cursor.close()
    conn.close()
    return render_template('booking_list.html', bookings=booking_list_items)
